...
def reset_system_metrics():
    global system_metrics, log_history, history_records
    ...
    empty_rates_data = {
        ...
        "Share (%)": ["0%", "0%", "0%", "0%", "0%"],
        "Bin Status": [
            f"0/{BIN_CAPACITIES['Plastic']} (0%)",
            f"0/{BIN_CAPACITIES['Glass']} (0%)",
            f"0/{BIN_CAPACITIES['Metal']} (0%)",
            f"0/{BIN_CAPACITIES['Paper/Cardboard']} (0%)",
            f"0/{BIN_CAPACITIES['Waste']} (0%)",
        ],
    }
    df_rates = pd.DataFrame(empty_rates_data)
    df_chart = pd.DataFrame({"Upload ID": [0], "Total Sorted": [0]})
...
def advanced_robot_logic(detected_objects):
    ...
    if not valid_objects:
        return (
            None,
            "⚠️ All corresponding bins for detected waste in this image are full!",
        )
    ...
    bbox_str = target_object.get('bbox', 'N/A')
    log_msg = f"🤖 COMMAND: Sort [{target_object['class']}] with confidence {target_object['confidence']:.2%}, Command Sent To Arduino, Position: {bbox_str}"
...
def process_uploaded_image(uploaded_frame):
    ...
    if target:
        Arduino.send_to_arduino(str(target['class']), SERIAL_COMMANDS, arduino)
        system_metrics[target['class']] += 1
        system_metrics["total_count"] += 1
        system_metrics["confidence_sum"] += target['confidence']
        system_metrics["total_revenue"] += WASTE_VALUES.get(
            target['class'], 0.0
        )
    ...
    rates_data = {
        ...
        "Share (%)": [
            (
                f"{(system_metrics['Plastic']/total)*100:.1f}%"
                if total > 0
                else "0%"
            ),
            (
                f"{(system_metrics['Glass']/total)*100:.1f}%"
                if total > 0
                else "0%"
            ),
            (
                f"{(system_metrics['Metal']/total)*100:.1f}%"
                if total > 0
                else "0%"
            ),
            (
                f"{(system_metrics['Paper/Cardboard']/total)*100:.1f}%"
                if total > 0
                else "0%"
            ),
            (
                f"{(system_metrics['Waste']/total)*100:.1f}%"
                if total > 0
                else "0%"
            ),
        ],
    ...
    avg_conf_raw = (
        (system_metrics["confidence_sum"] / total) if total > 0 else 0.0
    )
    ...
    history_records.append(
        {"Upload ID": len(history_records) + 1, "Total Sorted": total}
    )
...
with gr.Blocks(
    title="Industrial Smart Waste Sorter (Upload Mode)", theme=gr.themes.Soft()
) as demo:
    gr.Markdown("""
    # 🏭 Industrial Smart Waste Sorting System (Image Upload Mode Model Detection)
    """)

    # --- Added Hardware Serial Control Row ---
    with gr.Row():
    ...
    with gr.Row():
        with gr.Column():
            metric_conf = gr.Textbox(
                label="🎯 Cumulative Accuracy", value="0.0%"
            )
        with gr.Column():
            metric_speed = gr.Textbox(
                label="📦 Total Processed Units", value="0 Units"
            )
        with gr.Column():
            metric_oee = gr.Textbox(
                label="📊 Last Detection Score", value="0.0%"
            )
        with gr.Column():
            metric_rev = gr.Textbox(
                label="💵 Economic Value Generated", value="$0.00"
            )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            control_logs = gr.Textbox(
                label="Environmental Priority Logs", lines=5, interactive=False
            )
